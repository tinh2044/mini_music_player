...
function renderSongs(option, listSongAlbum) {
  ...
  var htmls = listSongs.map(
    (song, index) =>
      `
    <li data-index="${index}" data-type="${
        song.type
      }" class="container__left-item ${index === 0 ? "playing" : ""}">
  ...
  $(".container__left-quantity-song").textContent = `${
    (listSongs.length > 1 && listSongs.length + " songs") ||
    listSongs.length + " song"
  }`;
...
function handlePlayMusic(option, listSongs) {
  ...
  const showTime = $(".container__player-progress");
  ...
  function handleEvents() {
    // Handle When Hover Progress Bar
    showTime.onmousemove = (e) => {
      handleTooltipTime(e);
    };
    audio.onplay = () => {
      songPlay();
  ...
  function handleTooltipTime(e) {
    let position = e.offsetX;
    tooltipTime.style.left = `${position - 12}px`;
    tooltipTime.style.opacity = 1;
    const present = position / showTime.offsetWidth;
    let time = Math.floor(audio.duration * present);
    let timeMin = Math.floor(time / 60).toString();
    let timeSec = time % 60;
    if (timeSec < 10) {
      timeSec = `0${timeSec}`;
    } else {
    ...
    tooltipTime.textContent = `${timeMin}:${timeSec}`;
    showTime.onmouseout = () => {
  ...
  function clickAlbumItem() {
    ...
    albumItem.forEach((item) => {
      item.onclick = function () {
        ...
        var listSongs;
        if (type != "all") {
          listSongs = Array.from(listMusics).filter(
            (itemMusic) => itemMusic.type === type
          );
        } else {
